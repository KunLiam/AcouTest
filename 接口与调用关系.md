...
| **声学测试** | 
| | 气密性测试 | `setup_airtightness_tab()` | 堵mic/不堵mic双阶段测试与频谱对比 |
| | 震音测试 | `setup_jitter_tab()` | 震音播放与主观结果记录 |
...
| | 账号登录 | `setup_account_login_tab()` | 账号密码输入辅助 |
| | OpenClaw | `setup_openclaw_tab()` | HTTP控制接口状态与日志 |
| **烧大象key** | 
...
| UI 操作 | 调用方法 | 文件 |
|---------|----------|------|
| 开始测试（堵/不堵选择） | `start_airtightness_test()` | ui_components.py |
| 停止测试 | `stop_airtightness_test()` | ui_components.py |
| 手动导入堵mic/不堵mic文件 | `browse_airtight_compare_file(mode)` | ui_components.py |
| 对比频谱查看 | `show_airtight_compare_waveforms()` / `open_airtight_spectrum_compare_viewer()` | ui_components.py |
| 波形框选回填选段 | `open_audio_waveform_viewer(..., on_selection_changed=...)` | ui_components.py |

#### 震音测试

| UI 操作 | 调用方法 | 文件 |
|---------|----------|------|
| 开始播放/停止播放 | `start_jitter_test()` / `stop_jitter_playback()` | ui_components.py |
| 结果记录按钮 | `set_jitter_result(...)`（同类回调） | ui_components.py |
...
| 开始/继续唤醒率测试 | `_start_wakeup100_test()` | ui_components.py |
| 语料路径解析（ok_google / ok_freebox / ok_homa） | `_resolve_wakeup100_corpus()`、`_load_wakeup100_files_art()`、`_load_wakeup100_files_sorted()` | ui_components.py |
| 设备端必选 AudioPlayer + 语料附加 APK 及拉起 | `_ensure_audioplayer_apk_on_device()`、`_resolve_wakeup_corpus_extra_apk()`、`_launch_wakeup_corpus_extra_app()` | ui_components.py |
| logcat 唤醒行匹配（随语料库） | `_hotword_log_markers_for_corpus()` → `_read_hotword_logcat()` | ui_components.py |
...
| UI 操作 | 调用方法 | 文件 |
|---------|----------|------|
| 选择账号并自动填充 | `setup_account_login_tab()` 内绑定回调 | ui_components.py |
| 输入账号/密码并执行下一步 | 账号登录子页内对应执行函数 | ui_components.py |

#### OpenClaw

| UI 操作 | 调用方法 | 文件 |
|---------|----------|------|
| 查看连接状态与请求日志 | `setup_openclaw_tab()` | ui_components.py |
| 外部 HTTP 调用控制动作 | `AcouTestControlApi` (`control_api.py`) | control_api.py |
| OpenClaw 触发 ADB 命令日志 | `log_openclaw_adb()` / `append_openclaw_log()` | audio_test_tool.py + devices_operations.py |
