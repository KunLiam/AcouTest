...
- **日志查看器**：点击“打开日志查看器”按钮会弹出一个独立窗口「日志查看器 (Logcat Viewer)」，以表格形式（时间、PID、TID、级别、标签、包名、消息）实时显示设备 logcat。弹窗内支持 Filter 关键词过滤、级别下拉（V/D/I/W/E/F）、自动滚动、暂停/继续、清空、开始查看/停止查看；底部状态栏显示总计/显示/运行中。与“开始抓取/停止抓取”独立：可同时抓取到文件并打开弹窗实时查看，或只使用其一。
