...
- **唤醒率多语料**：「唤醒监测」中「100 条唤醒率测试」增加语料库下拉（ok_google / ok_freebox / ok_homa）。`ok_google` 使用 `wakeup_count/ok_google/art_100.txt` 顺序；其它语料目录内 wav 按路径排序播放；仍兼容旧目录 `art_100.txt` + `selected_100/`。Freebox 支持 **`wakeup_count/ok_freebox_single/`** 整轨 wav + 界面填写条数，无需改配置。唤醒计数：Freebox 为 **`KardomeJni: Keyword recognized!`**（B 版）及 **`Report wakeup phrase…-> ok_freebox`** / **`Received wake-up event: 1`** / **`kws wakeup`** / **`VOICE_PREPROCESS_WAKEUP sent phrase=ok_freebox`**（A 版 Elevoc 链路等）；**Homa** 仅 **`Received wake-up event: 1`**；Google 仍为 `Detected hotword` / `LIBAS_HOTWORD_DETECTION_RECEIVED`。必选 **`wakeup_count/AudioPlayer.apk`**；**ok_homa** 另需 **`ok_homa_31.apk`**；**ok_freebox** 仅当设备**未**安装 **com.kardome.audiodemo** 时需 **`ok_freebox_32.apk`**，开始测试时额外 **install -r**。
...
- **波形查看器（播放进度与时间轴对齐）**
  ...
  - 工具栏「振幅统计」旁新增「频率分析」：Welch 频谱；首标签 **全部通道** 可同图叠加多路曲线，亦可切换单 CH；**振幅统计与频率分析均不设独占 grab，可同时打开**；多通道频谱在后台线程并行计算，窗口先出、算完再画图。左侧 dB 刻度区 **手型光标**、**拖动平移** 纵轴、**滚轮缩放**（约 -400～+6 dB），双击恢复 0～-125 dB；切换 Notebook 标签时会重绘，避免隐藏页画布尺寸过小。
  - 「振幅统计」含 **总体 / 最大 / 最小 / 平均 RMS（dBFS）**（约 10 ms 分窗）；底部说明已缩短，按钮单独一行避免遮挡「复制表格」。
  - 气密性「频谱对比」窗口同样支持上述纵轴平移/缩放/双击恢复，导出 PNG 使用当前纵轴范围。
...
- **`ok_freebox_32.apk`**、**`ok_homa_31.apk`**：放在 **`wakeup_count/`** 根目录。**ok_homa** 开始测试时会安装/拉起 **ok_homa_31.apk**。**ok_freebox** 仅当设备**未**安装 **com.kardome.audiodemo** 时才安装 **ok_freebox_32.apk** 并拉起（见 1.2）。
...
3. **批量唤醒率测试**  
   ...
   - **统计行**：**全程合计** = 计划累计条数（与每档基准条数、音量档数、音效轮数有关；整轨模式下为「整轨条数 × 档数 × 音效轮数」等，详见界面「说明」按钮旁提示）；**基准** 为当前音量档的分母；**已唤醒** / **唤醒率** 按「当前档开始监测后的增量」实时更新（不再误用全局累计次数），唤醒率显示上限 100%。  
